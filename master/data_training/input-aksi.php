...
$cek_result = mysqli_query($connect,"SELECT * FROM nbc_responden where id_responden ='$id_responden' ")or die(mysqli_error());
...
$numrow = mysqli_num_rows($cek_result);
	...
	if($numrow == 0){
		$input_responden = "INSERT INTO nbc_responden VALUES('$id_responden','$nama') ";
	$responden_result = $db->query($input_responden);
	for($i=1;$i<=$jml_atribut;$i++){
		$a=$noatribut[$i];
		$b=$tampung[$i];
	$input_data = "INSERT INTO nbc_data (id_data,id_responden,id_atribut,id_parameter) VALUES(NULL,'$id_responden','$a','$b') ";
