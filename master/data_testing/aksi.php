...
$cek_id= "SELECT * FROM nbc_responden_test where id_responden ='$id_responden_test' ";
...
while($numrow > 0){
    ...
    $input_responden = "INSERT INTO nbc_responden_test VALUES('$id_responden_test','$nama','$alamat') ";
    $responden_result = $db->query($input_responden);

    for($i=1;$i<=$jml_atribut;$i++){
        $a=$noatribut[$i];
        $b=$tampung[$i];
    $input_data = "INSERT INTO nbc_data_test (id_data,id_responden,id_atribut,id_parameter) VALUES(NULL,'$id_responden_test','$a','$b') ";
